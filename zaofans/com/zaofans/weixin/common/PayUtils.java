...
public class PayUtils {
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public static String paySignSHA1(Map<String, String> params,String key) throws UnsupportedEncodingException {
		String stringA;
		try {
			stringA = createSign(params, false);
		} catch (UnsupportedEncodingException ex) {
		...
		String stringSignTemp = stringA + (key==null?"":("&key=" + key)) ;// 需要从配置中取出key值
		String signValue = DigestUtils.sha1Hex(stringSignTemp).toLowerCase();
		return signValue;
	}
	/**
	 * 判断请求是不是来在于微信
